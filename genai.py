...
def create_prompt_with_code_request(df_summary, df_detailed):
    ...
    Ensure the code checks if date columns are already in datetime format and, if not, converts them using `pd.to_datetime()`. Use these exact variable names. Respond with the code only, without any additional formatting or code block delimiters, so it can be run directly.
    """
    return prompt
